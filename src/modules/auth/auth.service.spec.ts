...
describe('AuthService', () => {
  ...
  describe('register', () => {
    ...
    it('deve lançar ConflictException se email já existir', async () => {
      ...
      await expect(
        service.register({ name: 'X', email: 'test@test.com', phone: '11900000000', password: '123' }),
  ...
  describe('login', () => {
    ...
    it('deve lançar UnauthorizedException se usuário não existir', async () => {
      ...
      await expect(
        service.login({ email: 'naoexiste@test.com', password: '123' }),
      ).rejects.toThrow(UnauthorizedException);
    });

    it('deve lançar UnauthorizedException se conta estiver inativa', async () => {
      const hash = await bcrypt.hash('Senha@123', 10);
      mockUsersService.findByEmail.mockResolvedValue({ ...mockUser, passwordHash: hash, isActive: false });
  ...
  describe('refresh', () => {
    it('deve retornar novos tokens com refresh token válido', async () => {
      mockJwtService.verify.mockReturnValue({ sub: 'uuid-1', email: 'test@test.com', role: Role.CLIENT });
