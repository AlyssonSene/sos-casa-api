...
export class PaymentsService {
  ...
  constructor(
    ...
    if (secretKey) {
      ...
      this.logger.warn(
        '[Stripe] STRIPE_SECRET_KEY não configurado — pagamentos via Stripe desabilitados',
      )
    }
  }
  ...
  async createPayment(
    ...
    professionalPixKey?: string,
    professionalPixKeyType?: string,
  ): Promise<Payment> {
  ...
  private async initManualPix(
    ...
    this.logger.log(
      `[PIX Manual] Pagamento ${payment.id} aguardando comprovante. Chave: ${pixKeyType}:${pixKey}`,
    )
  }
  ...
  async submitPixReceipt(requestId: string, attachmentId: string): Promise<Payment> {
    ...
    if (payment.status !== PaymentStatus.PROCESSING) {
      throw new BadRequestException(
        `Status atual (${payment.status}) não permite envio de comprovante`,
      )
    }
  ...
  async confirmPixByProfessional(requestId: string, professionalUserId: string): Promise<Payment> {
    ...
    if (payment.status !== PaymentStatus.HELD) {
      throw new BadRequestException(
        'Nenhum comprovante pendente de confirmação para este pagamento',
      )
    }
    ...
    this.logger.log(
      `[PIX Manual] Profissional ${professionalUserId} confirmou recebimento para payment ${payment.id}`,
    )
    return payment
  }

  private async createManualPixTransaction(
    payment: Payment,
  ...
  private scheduleEscrowRelease(requestId: string, hours: number): void {
    ...
    setTimeout(() => {
      void (async () => {
        ...
        if (payment?.status === PaymentStatus.HELD) {
          this.logger.log(
            `[Escrow] Auto-liberando pagamento do request ${requestId} após ${hours}h`,
          )
  ...
  async findByRequest(requestId: string): Promise<Payment> {
    const payment = await this.paymentRepo.findOneBy({ requestId })
    if (!payment)
      throw new NotFoundException(`Pagamento para a solicitação ${requestId} não encontrado`)
    return payment
  }
