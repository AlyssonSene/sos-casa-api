...
export class Payment {
  ...
  @Column({ name: 'stripe_payment_intent_id', type: 'varchar', nullable: true, length: 100 })
  stripePaymentIntentId: string | null
  @Column({ name: 'stripe_client_secret', nullable: true, type: 'text' }) stripeClientSecret:
    string | null
  @Column({ name: 'stripe_charge_id', type: 'varchar', nullable: true, length: 100 })
  stripeChargeId: string | null

  // ── PIX manual ────────────────────────────────────────────────────────────
  /** Snapshot da chave PIX do profissional no momento do pagamento */
  @Column({ name: 'manual_pix_key', type: 'varchar', nullable: true, length: 255 }) manualPixKey:
    string | null
  @Column({ name: 'manual_pix_key_type', type: 'varchar', nullable: true, length: 20 })
  manualPixKeyType: string | null
  /** UUID do attachment com o comprovante enviado pelo cliente */
  @Column({ name: 'receipt_attachment_id', type: 'uuid', nullable: true }) receiptAttachmentId:
    string | null
  /** Quando o profissional confirmou o recebimento */
  @Column({ name: 'professional_confirmed_at', type: 'timestamp', nullable: true })
