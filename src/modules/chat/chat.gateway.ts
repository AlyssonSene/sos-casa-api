...
interface SendMessagePayload { requestId: string; content: string; type?: MessageType; }
interface JoinRoomPayload { requestId: string; }
...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleMessage(@MessageBody() payload: SendMessagePayload, @ConnectedSocket() client: Socket) {
    ...
    const room = await this.chatService.findOrCreateRoom(payload.requestId);
    const message = await this.chatService.saveMessage(room.id, senderId, payload.content, payload.type);
