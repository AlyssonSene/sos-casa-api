...
describe('ServiceRequestsService', () => {
  ...
  describe('cancel', () => {
    it('deve cancelar solicitação em status PENDING', async () => {
      ...
      const result = await service.cancel('req-1', 'client-1', 'Desisti');
      expect(mockRepo.save).toHaveBeenCalledWith(
  ...
  describe('create', () => {
    it('deve criar solicitação com clientId', async () => {
      ...
      const result = await service.create({ clientId: 'client-1', addressId: 'addr-1' });
      expect(mockRepo.create).toHaveBeenCalledWith(expect.objectContaining({ clientId: 'client-1' }));
    });
  });
});
