...
export class ServicesService {
  constructor(
    ...
    @InjectRepository(ProfessionalService) private readonly proServiceRepo: Repository<ProfessionalService>,
  ) {}

  findAll(categoryId?: string): Promise<Service[]> {
    const where = categoryId ? { categoryId, isActive: true } : { isActive: true };
    return this.serviceRepo.find({ where, relations: { category: true }, order: { sortOrder: 'ASC' } });
  }
  ...
  findProfessionalServices(professionalId: string): Promise<ProfessionalService[]> {
    return this.proServiceRepo.find({ where: { professionalId, isActive: true }, relations: { service: true } });
  }

  async upsertProfessionalService(professionalId: string, dto: UpsertProfessionalServiceDto): Promise<ProfessionalService> {
    const existing = await this.proServiceRepo.findOneBy({ professionalId, serviceId: dto.serviceId });
    if (existing) {
      existing.price = dto.price;
      existing.isActive = true;
      return this.proServiceRepo.save(existing);
    }
    return this.proServiceRepo.save(this.proServiceRepo.create({ professionalId, serviceId: dto.serviceId, price: dto.price }));
  }
